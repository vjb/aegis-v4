...
var performStaticAnalysis = (nodeRuntime, input) => {
  ...
  if (mockData) {
    if (mockData.goplus.is_open_source === "0")
      unverifiedCode = 1;
    if (mockData.goplus.cannot_sell_all === "1")
      sellRestriction = 1;
...
var onAuditTrigger = (runtime2, log) => {
  ...
  try {
    if (log.data && log.data.length > 0) {
      ...
      if (rawHex.length >= 128) {
        ...
        let decoded = "";
        for (let i2 = 0;i2 < strHex.length; i2 += 2) {
          decoded += String.fromCharCode(parseInt(strHex.substring(i2, i2 + 2), 16));
        }
        if (decoded.startsWith("{"))
          firewallConfig = decoded;
      }
    }
  } catch {}
  ...
  try {
    ...
    if (typeof parsed.maxTax === "number")
      maxTax = parsed.maxTax;
    if (typeof parsed.blockProxies === "boolean")
      blockProxies = parsed.blockProxies;
    if (typeof parsed.strictLogic === "boolean")
      strictLogic = parsed.strictLogic;
    if (typeof parsed.blockHoneypots === "boolean")
  ...
  const reportDataHex = encodeAbiParameters(parseAbiParameters("uint256, uint256"), [tradeId, BigInt(riskMatrix)]);
  ...
  const txRes = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.vaultAddress,
  ...
  runtime2.log(`✅ onReport delivered to AegisModule at ${runtime2.config.vaultAddress}`);
...
var initWorkflow = (config) => {
  ...
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  return [
    handler(evmClient.logTrigger({ addresses: [hexToBase64(config.vaultAddress)] }), onAuditTrigger)
  ];
};
async function main() {
